...
Benefits:
- When we have multiple dispatches, if we leave that to the user, they can mis a call.
- If we pass this function in our custom hook or our provider it will become a part of 
react lifecycle update method (Example: We have to use this value in useEffect, we will need to memoize it), 
that is why we leave it outside. 
- Also, we can lazy load or code split. 
## Background
